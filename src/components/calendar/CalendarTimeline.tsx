...
export function CalendarTimeline({ selectedDate, onDateSelect }: CalendarTimelineProps) {
    ...
    useEffect(() => {
        ...
        const todayIndex = dates.findIndex(date => formatDateISO(date) === todayISO);
        if (todayIndex !== -1 && scrollViewRef.current) {
            // Center today in the viewport
            const scrollPosition = todayIndex * DAY_WIDTH - (SCREEN_WIDTH / 2) + (DAY_WIDTH / 2);
            setTimeout(() => {
